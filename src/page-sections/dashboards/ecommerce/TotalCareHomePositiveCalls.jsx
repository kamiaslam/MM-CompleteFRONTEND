...
export default function TotalCareHomePositiveCalls() {
  const { sentimentData, loading, error } = useCarehomeSentiment(); // Access the context

 // if (loading) return <div>Loading...</div>;
  if (error) return <div>Error: {error.message}</div>;

  const positiveCalls = sentimentData?.sentiment.positives || 0; // Adjust based on API response

  return (
    <Card className="p-3">
      <div>
        <FlexBox alignItems="center" gap={1}>
        {loading? <div>Loading...</div> : <H6>{format(positiveCalls)}</H6>}
        </FlexBox>
